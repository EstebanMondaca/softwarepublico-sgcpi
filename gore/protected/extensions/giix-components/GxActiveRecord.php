...
abstract class GxActiveRecord extends CActiveRecord {
    //RCP -- FUNCION QUE SIRVE PARA ALMACENAR EL LOG DE LAS TABLAS QUE SON CREADAS CON GIIX    
	 ...
	 * Notas:
	 * ====================
	 * Modelo Indicadores = Contiene una dependiencia hacia HitosIndicadores el cual solo depende de Indicadores.
	 * 						No posee un mantenedor, por lo tanto en la validacion, saltamos a este item.
	 */
	
	public function beforeDelete()
    {
        //$this->addError('atributo','Error al borrar');
        //luego debo recurperar el error así:
        //$error = $model->getError('atributo');
        $actualizarEstado=true;
        $tabla = self::model(get_class($this));
        $campoYtabla="<strong>Existen dependencias que deben ser resueltas antes de eliminar el registro.</strong>";
        $campoYtabla.="<ul style='height: 150px; overflow: auto; width: 98%;'>";
        foreach($tabla->relations() as $k=>$v)
        {             
            //Validando el tipo de relación
            if($v[0]=='CHasManyRelation' || $v[0]=='CManyManyRelation'){	
                foreach($this->$k as $relRecord) {
                		//Consultamos por el nombre del objeto y comparamos
                		if(isset($relRecord->estado)){
                		    if (get_class($relRecord) != 'HitosIndicadores' && get_class($relRecord) != 'HitosActividades' && get_class($this) != 'ObjetivosEstrategicos' && get_class($this) != 'ProductosEstrategicos'){
                                if($relRecord->estado=='1'){
                                    $actualizarEstado=false;    
                                    if($v[0]=='CManyManyRelation'){
                                        $campoYtabla.="<li>Existe una relación asociada entre '".$relRecord->label(1)."' y '".$this->label(1)."' en el registro: '".$relRecord."'</li>";
                                    }else{
                                        $campoYtabla.="<li>Existe una dependencia asociada a '".$relRecord->label(1)."' en el registro: '".$relRecord."'</li>";
                                    }                                  
                                }
                            }
                            if(get_class($this) == 'ObjetivosEstrategicos' && get_class($relRecord)=='ProductosEstrategicos'){
                                if($relRecord->estado=='1'){
                                    $actualizarEstado=false;
                                    $campoYtabla.="<li>Existe una relación asociada entre '".$relRecord->label(1)."' y '".$this->label(1)."' en el registro: '".$relRecord."'</li>";
                                }
                            ...
                            if(get_class($this) == 'ProductosEstrategicos'){
                                if($relRecord->estado=='1' && get_class($relRecord)!='ObjetivosEstrategicos' && get_class($relRecord)!='Clientes'){
                                    $actualizarEstado=false;    
                                    if($v[0]=='CManyManyRelation'){
                                        $campoYtabla.="<li>Existe una relación asociada entre '".$relRecord->label(1)."' y '".$this->label(1)."' en el registro: '".$relRecord."'</li>";
                                    }else{
                		...
                		}else if(!isset($relRecord->estado) && $v[0]=='CManyManyRelation'){
                		    $actualizarEstado=false;
                		    if(get_class($relRecord) == 'User'){
                		        $campoYtabla.="<li>Existe una relación asociada entre '".$relRecord->label(1)."' y '".$this->label(1)."' en el registro: '".$relRecord->username."'</li>";
                		    }else{
                		        $campoYtabla.="<li>Existe una relación asociada entre '".$relRecord->label(1)."' y '".$this->label(1)."' en el registro: '".$relRecord."'</li>";
                		    }      
                		}
        ...
        //Validamos que si queremos eliminar una linea de acción no contenga inconvenientes de lo contrario no se debe cambiar el estado del indicador asociado
        if(get_class($this)=='LineasAccion' && $actualizarEstado){
            $modelIndicadores= Indicadores::model()->find("id=".$this->id_indicador);
