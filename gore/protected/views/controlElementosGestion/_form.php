...
<?php 
    ...
    Yii::app()->clientScript->registerScriptFile(Yii::app()->request->baseUrl.'/js/jquery.MultiFile.js'); 
	...
	?>
        ...
        <table width="100%" border="0" cellspacing="5" cellpadding="5" class="control">
            ...
            <td valign="top" width="23%" rowspan="3">
                <div class="fieldset2" style='height: 428px;'>
                                          ...
                                          if(empty($v->id_la) && $v->idPlanificacion->id==Yii::app()->session['idPlanificaciones']){
                                              ...
                                              echo "<td>V".$x."</td>";
                                                 if($v->puntaje_revisado!=null && $v->puntaje_revisado!=''){
                                                    ...
                                                    echo "<td><input type='text' name='laElemGestions[".$v->id."][fecha]' class='datepicker' value='".$anioInput."' style='height: 15px; width: 85px;'/></td>";
                                                 }
                                              echo "</tr>";
                                              $x++;  
                                          }
                    ...
                    <div> 
                        ...
                        <input type="button" value="Agregar Nuevo" name="yt0" onclick="agregarNuevaVersionRegistro();">
                        ...
                        foreach($model->laElemGestions as $v):
                            if($v->idPlanificacion->id==Yii::app()->session['idPlanificaciones']){
                                if($v->puntaje_revisado!=null && $v->puntaje_revisado!=''){
                                    ...
                                    echo '<textarea class="laElemGestions_'.$v->id.'" id="laElemGestions_'.$v->id.'_evidencia" name="laElemGestions['.$v->id.'][evidencia]" rows="3" style="width: 390px;display:none;height: 169px;">'.$v->evidencia.'</textarea>';
                                }
                            }
                        endforeach;
                    ?>
          ...
          <tr>
            ...
            <td valign="top" colspan="2" rowspan="2">
                 <div id="laElemGestions_archivo" class="fieldset2">
                    ...
                    <?php  
                    		//print_r($model->laElemGestions->laElemGestionDoc); 
				       		foreach($model->laElemGestions as $v):
				       		...
				       		if($v->idPlanificacion->id==Yii::app()->session['idPlanificaciones']){
				       			...
				       			if($v->puntaje_revisado!=null && $v->puntaje_revisado!=''){
									...
									echo '<b class="laElemGestions_'.$v->id.' file"> Subir Documentos :</b>';
						       		echo "<div class='laElemGestions_".$v->id." file' style='background: none repeat scroll 0 0 #DADADA; width: 248px;'>";
				       				$this->widget('CMultiFileUpload',array(
							           'name'		=>'laElemGestions['.$v->id.'][archivo]',
							           'accept'		=>'pdf|doc|xls|docx|xlsx|rar|zip',
							           'max'		=>10,
							           'remove'		=>Yii::t('ui',' X '),
							           'denied'		=>'Tipo de archivo no está permitido.', //message that is displayed when a file type is not allowed
							           'duplicate'	=>'Archivo seleccionado anteriormente.', //message that is displayed when a file appears twice
							           'htmlOptions'=>array('size'=>19,'class'=>'laElemGestions_'.$v->id),
							       	   'id'			=>'laElemGestions_'.$v->id.'_archivo',
						       		)); 
						       		echo "</div><!--fin widget-->";
				       				$modelDocument = LaElemGestionDocumentos::model()->findAll(array('condition'=>'t.la_elem_id='.$v->id .' AND estado = 1'));
				       				if (isset($modelDocument[0]['nombre'])){
				       					...
				       					foreach($modelDocument as $itemsDoc){
											$nombreDoc = $itemsDoc->nombre;
				       						$temp_nombre= 	substr($nombreDoc,14);
					       					 $nombre_archivo = substr($temp_nombre,0, 20);
					       					 if(strlen($temp_nombre)>20){$nombre_archivo .= "...";}
											 echo '<tr class="laElemGestions_'.$itemsDoc->la_elem_id.' file" id="laElemGestions_'.$itemsDoc->id.'_archivo" style="display:none;"><td>'.$nombre_archivo.'</td><td><a href="'.$origenUrl.$itemsDoc->nombre.'" target="_blank"><img src="'.Yii::app()->request->baseUrl.'/images/view.png"/></a></td><td><a href="#" onclick="borrarDocEvidenciaAlmacenado('.$itemsDoc->id.','.$itemsDoc->la_elem_id.')"><img src="'.Yii::app()->request->baseUrl.'/images/delete.png"/></a></td></tr>';
				       					}
				       					echo '</table></div>';
				       				}
				       			}
                    	...
                    	foreach($model->laElemGestions as $v):
                            if($v->idPlanificacion->id==Yii::app()->session['idPlanificaciones']){
                                ...
                                if($v->puntaje_revisado!=null && $v->puntaje_revisado!=''){
                                    ...
                                    echo '<input type="file" class="laElemGestions_'.$v->id.'" id="laElemGestions_'.$v->id.'_archivo" name="laElemGestions['.$v->id.'][archivo]" style="display:none; width: 233px;">';
          ...
          <tr>
            <td>
                <?php 
                       // $idPlanificacion=Planificaciones::model()->findAll(array('condition'=>'t.estado=1 AND pp.descripcion="'.Yii::app()->session['idPeriodoSelecionado'].'"', 'join'=>'inner join periodos_procesos pp on t.periodo_proceso_id=pp.id'));                    
                        //echo '<input type="hidden" value="'.$model->laElemGestions[0]->id_la.'" name="id_la">';
                        ...
                        if($model->laElemGestions){
                            ...
                            $responsableID=(isset($model->elementosGestionResponsables[0]))?$model->elementosGestionResponsables[0]->responsable_id:"";
                            
                            echo '<input type="hidden" value="'.$responsableID.'" name="responsable_id">';   
                        }else{
                            echo '<input type="hidden" value="0" name="puntaje_esperado">';  
                            echo '<input type="hidden" value="'.Yii::app()->user->id.'" name="responsable_id">';   
                        }      
                        echo '<input type="button" value="Guardar" onclick="validarcontrolLaElemGestion();">';                                      
                ?>
            </td>
          </tr>
        </table>	
