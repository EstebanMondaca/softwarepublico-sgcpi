...
class MailboxModule extends CWebModule
{
	...
	*/
	 ...
	 */
	public $defaultSubject = '(Sin asunto)';
	/**
	 ...
	 */
	public $allowableCharsSubject = '0-9a-z.áéíóúÁÉÍÓÚñÑ,!?@°\s*$%#&;:+=_(){}\[\]\/\\-';
	/**
	...
	*/
	...
	{
		return Yii::app()->createUrl('user/admin/view/', array('id' => $userid));
	}
	...
	*/
	public function autoComplete($term)
	{
		...
		foreach($users as $user)
		{
			...
			$json .= '{"label":"'.$user->{$this->usernameColumn}.' ('.$user->nombres.' '.$user->ape_paterno.')",'
	...
	public function getDate($time)
	{
		$timediff = time() - $time;
		if($timediff < 60 )
			$date = $timediff . ' seg atrás';
		elseif($timediff < 3600 - 60) // within last hour
			$date = ceil($timediff / 60) . ' min atrás';
		elseif($time > strtotime('today')) // today
			$date = date('h:i a',$time);
		elseif($time > strtotime(date('Y-1-01'))) // within this year
			$date =  Yii::app()->dateFormatter->format('dd MMM', $time);//date('M j',$time);
		else
			$date = Yii::app()->dateFormatter->format('dd MMM, yyyy', $time);//date('m/d/y',$time); // last year or more
		return $date;
	}
