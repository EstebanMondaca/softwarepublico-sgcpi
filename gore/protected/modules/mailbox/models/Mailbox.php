...
class Mailbox extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('initiator_id,tipo_mensaje_id, interlocutor_id, bm_read, bm_deleted, initiator_del, interlocutor_del', 'numerical', 'integerOnly'=>true),
	...
	public function relations()
	{
		...
		return array(
		            ...
		            'tipoMensaje'=>array(self::BELONGS_TO, 'TiposMensajes','tipo_mensaje_id'),
		);
	}
	...
	public function attributeLabels()
	{
		return array(
			'conversation_id' => 'Id',
			'initiator_id' => 'Desde Id',
			'interlocutor_id' => 'Para Id',
			'to' => 'Para',
			'subject' => 'Asunto',
			'last_message_ts' => 'Último mensaje recibido',
			'modified' => 'Último modificado',
			'is_system' => 'Es la plataforma',
			'tipoMensaje'=>'Tipo',
			'tipo_mensaje_id'=>'Tipo',
		);
	}
	...
	public function inbox($userid)
	{
		$this->getDbCriteria()->mergeWith(array(
			...
			m.text, m.created AS last_message_ts, ms.is_replied, 
			c.conversation_id, c.initiator_id, c.interlocutor_id, c.subject, c.bm_read, 
			c.bm_deleted, c.modified, c.is_system,d.nombre as tipoMensaje',
			'alias'=>'c',
			'join'=>"INNER JOIN (
				SELECT message_id,conversation_id, sender_id,recipient_id,text,created FROM ".MailboxModule::TBL_MSG." 
				WHERE recipient_id=:userid ORDER BY created DESC  
			) AS m ON(m.conversation_id=c.conversation_id) 
            LEFT JOIN tipos_mensajes d ON (d.id=c.tipo_mensaje_id)
			INNER JOIN (
				SELECT conversation_id, IF(sender_id=:userid,1,0) AS is_replied FROM ".MailboxModule::TBL_MSG." 
				WHERE (recipient_id=:userid OR sender_id=:userid) ORDER BY created DESC 
			) AS ms ON(ms.conversation_id=c.conversation_id)",
